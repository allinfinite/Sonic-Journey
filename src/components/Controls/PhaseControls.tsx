...
// Melody style options
const MELODY_STYLE_OPTIONS: { value: MelodyStyle; label: string }[] = [
...
// Scale options
const MELODY_SCALE_OPTIONS: { value: MelodyScale; label: string }[] = [
  { value: 'pentatonic_minor', label: 'Pent. Minor' },
  { value: 'pentatonic_major', label: 'Pent. Major' },
...
// Density options
const MELODY_DENSITY_OPTIONS: { value: NoteDensity; label: string }[] = [
      ...
      <div className="space-y-2 pt-2 border-t border-[var(--color-surface-light)]">
        <div className="flex items-center justify-between">
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              type="checkbox"
              checked={phase.melody_enabled !== false && (journey.layers?.melody_layer === true)}
              onChange={(e) => {
                const enabled = e.target.checked;
                updatePhase(selectedPhaseIndex, { melody_enabled: enabled });
              }}
              className="sr-only peer"
            />
            <div className="w-11 h-6 bg-[var(--color-surface-light)] peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[var(--color-primary)]"></div>
        ...
        {phase.melody_enabled !== false && journey.layers?.melody_layer === true && (
          <div className="space-y-3 pl-6">
            {/* Style Selection */}
            <div className="space-y-1">
              <label className="text-xs text-[var(--color-text-muted)]">Style</label>
              <div className="grid grid-cols-5 gap-1">
                {MELODY_STYLE_OPTIONS.map((option) => (
                  <button
                    key={option.value}
                    onClick={() => updatePhase(selectedPhaseIndex, { melody_style: option.value })}
                    className={`px-2 py-1 rounded text-xs font-medium transition-colors ${
                      (phase.melody_style || 'mixed') === option.value
                        ? 'bg-[var(--color-primary)] text-white'
                        : 'bg-[var(--color-surface-light)] text-[var(--color-text-muted)] hover:bg-[var(--color-surface-light)]/80'
                    }`}
                  >
            ...
            <div className="space-y-1">
              ...
              <div className="grid grid-cols-5 gap-1">
                {MELODY_SCALE_OPTIONS.map((option) => (
                  <button
                    key={option.value}
                    onClick={() => updatePhase(selectedPhaseIndex, { melody_scale: option.value })}
                    className={`px-2 py-1 rounded text-xs font-medium transition-colors ${
                      (phase.melody_scale || 'pentatonic_minor') === option.value
                        ? 'bg-[var(--color-primary)] text-white'
                        : 'bg-[var(--color-surface-light)] text-[var(--color-text-muted)] hover:bg-[var(--color-surface-light)]/80'
                    }`}
                  >
            ...
            {/* Density Selection */}
            <div className="space-y-1">
              <label className="text-xs text-[var(--color-text-muted)]">Density</label>
              <div className="grid grid-cols-3 gap-1">
                {MELODY_DENSITY_OPTIONS.map((option) => (
                  <button
                    key={option.value}
                    onClick={() => updatePhase(selectedPhaseIndex, { melody_density: option.value })}
                    className={`px-2 py-1 rounded text-xs font-medium transition-colors ${
                      (phase.melody_density || 'moderate') === option.value
                        ? 'bg-[var(--color-primary)] text-white'
                        : 'bg-[var(--color-surface-light)] text-[var(--color-text-muted)] hover:bg-[var(--color-surface-light)]/80'
                    }`}
                  >
            ...
            {/* Intensity Slider */}
            <div className="flex items-center gap-2">
              <label className="text-xs text-[var(--color-text-muted)] w-12">Volume</label>
              <input
                type="range"
                min="0"
                max="100"
                ...
                onChange={(e) => updatePhase(selectedPhaseIndex, { melody_intensity: Number(e.target.value) / 100 })}
                className="flex-1 h-1.5 bg-[var(--color-surface-light)] rounded-lg appearance-none cursor-pointer accent-[var(--color-accent)]"
              />
              <span className="text-xs text-[var(--color-text)] w-8">{Math.round((phase.melody_intensity ?? 0.3) * 100)}%</span>
            </div>
          </div>
        )}

        {/* Note if melody layer is globally disabled */}
        {phase.melody_enabled !== false && journey.layers?.melody_layer !== true && (
